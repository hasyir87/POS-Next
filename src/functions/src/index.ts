...
// Konfigurasi CORS untuk mengizinkan semua origin (baik untuk development)
// Dalam produksi, Anda mungkin ingin membatasinya ke domain frontend Anda.
const corsMiddleware = cors({ origin: true });

// Helper function to get authenticated user UID
...
export const createUser = functions.https.onRequest((request, response) => {
    corsMiddleware(request, response, async () => {
        ...
        try {
            ...
            if (!organizationId) {
                throw new functions.https.HttpsError("invalid-argument", "Organization ID is required to create a user.");
            }
...
export const createOutlet = functions.https.onRequest((request, response) => {
    corsMiddleware(request, response, async () => {
        ...
        try {
            const callingUid = await getAuthenticatedUid(request);
            const { outletName, parentOrganizationId } = request.body;
            if (!outletName || !parentOrganizationId) {
                throw new functions.https.HttpsError("invalid-argument", "Outlet name and parent organization ID are required.");
            }
