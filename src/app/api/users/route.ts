...
export async function POST(req: Request) {
  ...
  if (
    ...
    return NextResponse.json(
      {
        error:
          "Requesting user profile not found or not associated with an organization",
      },
      { status: 404 },
    );
  }
  ...
  if (role === "owner" && requestingProfile.role !== "superadmin") {
    return NextResponse.json(
      { error: "Forbidden: Only superadmin can assign owner role" },
      { status: 403 },
    );
  }
  ...
  if (password) {
    ({ data: userAuthData, error: authError } =
  ...
  if (authError) {
    console.error(
      "Error creating/inviting Supabase Auth user:",
      authError.message,
    );
  ...
  if (!userAuthData?.user) {
    return NextResponse.json(
      { error: "Supabase Auth user creation/invitation failed unexpectedly." },
      { status: 500 },
    );
  }
