...
export const createOwner = functions.https.onCall(async (data, context) => {
  ...
  if (!email || !password || !fullName || !organizationName) {
    throw new functions.https.HttpsError(
        "invalid-argument",
        "Data tidak lengkap. Pastikan semua field terisi."
    );
  }
  ...
  try {
    ...
    // --- Rollback logic ---
    if (newUserRecord) {
      await admin.auth().deleteUser(newUserRecord.uid).catch((err) =>
        functions.logger.error("Failed to rollback auth user creation:", err)
      );
    }
    const err = error as { code?: string; message: string };
    functions.logger.error("ERROR IN createOwner:", err);
    if (
    ...
    if (err.code === "already-exists") { // Re-throw our custom error
      throw err;
    }
    // Re-throw other errors to be caught by the client
    throw new functions.https.HttpsError(
        "internal",
        `Gagal membuat pemilik baru: ${err.message}`,
        err
    );
  }
});
...
// --- Cloud Function for an Owner/Admin to create a new user ---
export const createUser = functions.https.onCall(async (data, context) => {
  ...
  if (role === "owner" || role === "superadmin") {
    throw new functions.https.HttpsError(
        "permission-denied",
        "Anda tidak dapat membuat pengguna dengan peran ini."
    );
  }
  ...
  try {
    ...
    if (
      requestingProfile?.organization_id !== organizationId &&
      requestingProfile?.role !== "superadmin"
    ) {
      throw new functions.https.HttpsError(
          "permission-denied",
          "Anda tidak dapat membuat pengguna untuk organisasi lain."
      );
    }
    if (
    ...
    const err = error as { code?: string; message: string };
    functions.logger.error("ERROR IN createUser:", err);
    ...
    throw new functions.https.HttpsError(
        "internal",
        `Gagal membuat pengguna baru: ${err.message}`,
        err
    );
  }
});
...
export const deleteUser = functions.https.onCall(async (data, context) => {
  ...
  if (!uidToDelete) {
    throw new functions.https.HttpsError(
        "invalid-argument",
        "User ID yang akan dihapus tidak disediakan."
    );
  }
  ...
  try {
    ...
    const [requesterProfileSnap, userToDeleteProfileSnap] = await Promise.all([
      requesterProfileRef.get(),
      userToDeleteProfileRef.get(),
    ]);
    ...
    if (!userToDeleteProfileSnap.exists) {
      await admin.auth().deleteUser(uidToDelete).catch((err) =>
        functions.logger.warn("Auth user not found, nothing to delete.", err)
      );
      return {
        status: "success",
        message: "Profil pengguna tidak ditemukan, akun login (jika ada) telah dihapus.",
      };
    }
    ...
    if (
      requesterProfile?.organization_id !==
        userToDeleteProfile?.organization_id &&
      requesterProfile?.role !== "superadmin"
    ) {
      throw new functions.https.HttpsError(
          "permission-denied",
          "Anda tidak dapat menghapus pengguna dari organisasi lain."
      );
    }
    if (
    ...
    if (userToDeleteProfile?.role === "owner") {
      throw new functions.https.HttpsError(
          "permission-denied",
          "Akun pemilik tidak dapat dihapus melalui cara ini."
      );
    }
    ...
    functions.logger.error("ERROR IN deleteUser:", err);
    throw new functions.https.HttpsError(
        "internal",
        `Gagal menghapus pengguna: ${err.message}`,
        err
    );
  }
});
...
export const setupInitialData = functions.https.onCall(
    async (data, context) => {
      if (!context.auth) {
        throw new functions.https.HttpsError(
            "unauthenticated",
      ...
      try {
        ...
        if (!profileData || !profileData.organization_id) {
          throw new functions.https.HttpsError(
              "failed-precondition",
              "Pengguna tidak terhubung ke organisasi."
          );
        }
        ...
        if (orgData.is_setup_complete) {
          return {
            status: "success",
            message: "Toko sudah disiapkan sebelumnya.",
          };
        }
        ...
        const err = error as { message: string };
        functions.logger.error("ERROR IN setupInitialData:", err);
        throw new functions.https.HttpsError(
            "internal",
            `Gagal melakukan setup: ${err.message}`,
            err
        );
      }
    }
...
export const get_dashboard_analytics = functions.https.onCall(async (data, context) => {
  ...
  try {
    ...
    const err = error as { message: string };
    functions.logger.error("ERROR IN get_dashboard_analytics:", err);
    throw new functions.https.HttpsError("internal", `Gagal mengambil data analitik: ${err.message}`, err);
  }
});
