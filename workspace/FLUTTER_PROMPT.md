...
*   **Manajemen State**: Gunakan **`flutter_bloc`**. Ini adalah pilihan krusial untuk memastikan arsitektur yang skalabel, teruji, dan modular. Dengan memisahkan logika bisnis dari UI, penambahan atau perbaikan fitur di masa depan menjadi jauh lebih mudah tanpa mempengaruhi modul lain.
*   **Backend**: Supabase. Aplikasi Flutter akan berinteraksi dengan Supabase menggunakan paket `supabase_flutter`.
*   **Navigasi**: Gunakan `GoRouter` untuk manajemen rute yang kuat dan berbasis URL, yang memudahkan deep linking dan navigasi modular.
...
*   **Dependensi Penting**:
    ...
    *   `equatable`: Untuk perbandingan objek yang efisien dalam state BLoC.
...
-   **Struktur Proyek Modular**: Gunakan struktur direktori yang bersih dan terorganisir. Pisahkan kode berdasarkan fitur (`features/pos`, `features/inventory`, dll.) atau berdasarkan lapisan (`data/repositories`, `presentation/blocs`, `presentation/widgets`, `presentation/screens`). Ini sangat penting untuk kemudahan perawatan dan pengembangan jangka panjang.
-   **Pemisahan Tanggung Jawab (Separation of Concerns)**: Patuhi pola BLoC dengan ketat.
    -   **UI (Widgets)** hanya boleh bertugas untuk menampilkan state dan mengirim event.
    -   **BLoC** berisi semua logika bisnis dan berinteraksi dengan repository.
    -   **Repository** bertanggung jawab untuk mengambil dan mengirim data dari Supabase.
-   **Penanganan Error yang Tangguh**: Implementasikan penanganan error yang baik untuk panggilan API. Gunakan state `error` dalam BLoC untuk menampilkan pesan yang informatif kepada pengguna melalui `SnackBar` atau dialog, tanpa membuat aplikasi crash.
-   **Loading State**: Selalu tampilkan indikator loading (misalnya, `CircularProgressIndicator`) saat data sedang diambil dari Supabase. Gunakan state `loading` di dalam BLoC.
-   **Kode yang Bersih**: Tulis kode yang bersih, dapat dibaca, dan mudah dikelola. Gunakan nama variabel dan fungsi yang jelas.
